<template>
    <div>
        ...
        >
            <template #actions>
                <AppButton variant="secondary" :to="{ name: 'admin.properties' }" size="sm">
        ...
        <template v-else>
            ...
            <div class="mb-4 rounded-xl border border-gray-200 bg-white">
                <div class="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4">
                    ...
                    <AppButton variant="secondary" size="sm" :to="{ name: 'admin.properties.edit', params: { uuid: property.uuid } }">Edit</AppButton>
                </div>
                <div class="px-5 py-5">
            ...
            <div class="mb-4 rounded-xl border border-gray-200 bg-white">
                <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                    ...
                    <AppButton size="sm" @click="showAddBlock = true">
                    ...
                    <div v-for="block in blocks" :key="block.uuid" class="px-5 py-3.5">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-1">
                                <button @click="startEditBlock(block)" class="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" />
                                    </svg>
                                </button>
                                <button @click="confirmDeleteBlock(block)" class="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <div v-if="expandedBlocks.has(block.uuid)" class="mt-3 ml-7 rounded-lg border border-gray-100 bg-gray-50">
                            <div class="flex items-center justify-between border-b border-gray-100 px-4 py-2.5">
                                ...
                                <button @click="startAddUnit(block)" class="flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-primary-600 transition-colors hover:bg-primary-50">
                            ...
                            <div v-else class="divide-y divide-gray-100">
                                <div v-for="unit in blockUnits[block.uuid]" :key="unit.uuid" class="flex items-center justify-between px-4 py-2.5">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <button @click="startEditUnit(block, unit)" class="rounded-lg p-1 text-gray-400 transition-colors hover:bg-gray-200 hover:text-gray-600">
                                            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" />
                                            </svg>
                                        </button>
                                        <button @click="confirmDeleteUnit(block, unit)" class="rounded-lg p-1 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500">
                                            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                            </svg>
                                        </button>
